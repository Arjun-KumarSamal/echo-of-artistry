...
import { Database, BarChart3, Cloud, Cpu } from 'lucide-react';
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div 
              key={project.id} 
              className="group backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-2xl"
            >
              <div className="relative overflow-hidden">
                ...
                <div className="absolute top-4 right-4 backdrop-blur-sm bg-white/20 border border-white/30 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center gap-2">
                  {project.icon}
                  {project.category}
                </div>
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag, index) => (
                    <span 
                      key={index}
                      className="backdrop-blur-sm bg-white/10 border border-white/20 text-gray-300 px-3 py-1 rounded-full text-xs font-medium hover:bg-purple-500/20 hover:text-purple-300 transition-colors"
                    >
                      {tag}
                    </span>
                  ))}
